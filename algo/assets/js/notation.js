/* Algorithms Tutor — plain-language symbol support. Same engine as the ML
   tutor's notation.js, because this course typesets math with KaTeX too.

   A page declares its symbols once:

     <script type="application/json" class="symbols">
       { "O": { "name": "O — “big O”", "pron": "say: big oh",
                "means": "an upper bound on how fast the cost grows" }, ... }
     </script>

   Then:
   - <ul class="symbol-legend" data-symbols="O,n,\\log"></ul> renders a chip row
     under an equation (one chip per symbol, click for details).
   - Any <span data-sym="O">O</span> in prose becomes clickable.
...
(function () {
  ...
  // per atom (e.g. <span class="mord mathnormal">n</span>), so exact text
  // matches are safe. Glyphs inside accent constructions are skipped —
