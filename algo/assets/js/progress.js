/* Algorithms Tutor — learner progress in localStorage.

   Same shape as the ML, bass and git tutors, in its own namespace so the
   courses never collide on one origin.

   Pages (key "algo-tutor:progress"):
     { "pages": { "<pageId>": { "completedAt": "<ISO date>",
                                "quiz": { "correct": 2, "total": 3 } } } }

   Labs (key "algo-tutor:labs") — same split as the git tutor, because a page
   here can have two independent halves, a quiz and a graded exercise:
     { "<labId>": { "doneAt": "<ISO>", "assisted": false, "pageId": "…", "runs": 3 } }

   Page ids are stable slugs declared by each page's quiz JSON; lab ids are
   globally unique and prefixed with the page slug (build-algo-index.js enforces
   both). The landing page uses this to offer "continue where you left off". */
(function () {
  "use strict";

  const KEY = "algo-tutor:progress";
  const KEY_LABS = "algo-tutor:labs";
  const KEY_MISSED = "algo-tutor:missed";
  ...
  window.AlgoProgress = {
    ...
    // Labs: the graded-exercise half of a page (engine lands milestone 7).
    ...
    // Missed quiz questions become flashcards (flashcards.html, milestone 8)
