/* Algorithms Tutor — declarative quizzes with instant feedback.
   ...
   Questions are asked in plain words (understanding, not vocabulary — see PLAN.md).
   Scores are stored via progress.js; finishing marks the page complete. */
(function () {
  "use strict";
  ...
  function render(root, quiz) {
    ...
    quiz.questions.forEach((question, qi) => {
      ...
      question.choices.forEach((choiceText, ci) => {
        ...
        btn.addEventListener("click", () => {
          ...
          if (isRight) correct++;
          else if (window.AlgoProgress && quiz.id) {
            window.AlgoProgress.recordMiss(
              quiz.id, question.q,
    ...
    function finish() {
      ...
      if (window.AlgoProgress && quiz.id) {
        window.AlgoProgress.recordQuiz(quiz.id, correct, quiz.questions.length);
        window.AlgoProgress.markComplete(quiz.id);
      }
    }
  }
