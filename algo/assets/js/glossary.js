/* Algorithms Tutor — glossary engine. Two jobs:

   1. On content pages: any <dfn data-term="frontier">frontier</dfn> becomes
      clickable and opens a popover — plain-English line first, precise
      definition second (the site's plain-language rule applies to the glossary
      itself).
   ...
   Entries live in data/glossary.json, keyed by slug:
     { "frontier": { "term", "plain", "definition", "also": [aliases],
                     "see": [slugs], "taughtIn": {href,title} } }

   Note: entries are fetched, so this needs an http server (npm run dev:algo).
   Over file:// the terms degrade gracefully to plain text. */
(function () {
  "use strict";

  const metaEl = document.querySelector('meta[name="algo:root"]');
  ...
  function renderGlossaryPage(app) {
    ...
    controls.innerHTML =
      `<input type="search" placeholder="Filter — try “worst case”, “frontier”, or a symbol like Θ"
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!glossary) {
      if (app) app.innerHTML =
        ...
        "start the dev server instead: <code>npm run dev:algo</code></p>";
      return;
    }
    buildDialog();
    wireTerms();
