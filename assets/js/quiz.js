/* ML Tutor — declarative quizzes with instant feedback.
...
(function () {
  ...
  function render(root, quiz) {
    ...
    function finish() {
      ...
      if (window.MLProgress && quiz.id) {
        window.MLProgress.recordQuiz(quiz.id, correct, quiz.questions.length);
        window.MLProgress.markComplete(quiz.id);
      }
    }
  }
