/* Git Tutor — glossary engine. Two jobs:

   1. On content pages: any <dfn data-term="loss">loss</dfn> becomes clickable and
      opens a popover — plain-English line first, precise definition second (the
      site's plain-language rule applies to the glossary itself).

   2. On glossary.htgit: renders the full glossary from data/glossary.json into
      #glossary-app, grouped alphabetically, with a live filter box.

   Entries live in data/glossary.json, keyed by slug:
     { "loss": { "term", "plain", "definition", "also": [aliases],
...
(function () {
  ...
  const metaEl = document.querySelector('meta[name="git:root"]');
  ...
  function renderGlossaryPage(app) {
    ...
    controls.innerHTML =
      `<input type="search" placeholder="Filter — try “grade”, “miss”, or a symbol like η"
