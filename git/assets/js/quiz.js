/* Git Tutor — declarative quizzes with instant feedback.
...
(function () {
  ...
  function render(root, quiz) {
    ...
    quiz.questions.forEach((question, qi) => {
      ...
      question.choices.forEach((choiceText, ci) => {
        ...
        btn.addEventListener("click", () => {
          ...
          if (isRight) correct++;
          else if (window.GitProgress && quiz.id) {
            window.GitProgress.recordMiss(
              quiz.id, question.q,
    ...
    function finish() {
      ...
      if (window.GitProgress && quiz.id) {
        window.GitProgress.recordQuiz(quiz.id, correct, quiz.questions.length);
        window.GitProgress.markComplete(quiz.id);
      }
    }
  }
